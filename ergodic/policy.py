...
class ShootingMPCPolicy(Policy):
    ...
    @abc.abstractmethod
    def batched_cost_train(self, trajectory: torch.Tensor, actions: torch.Tensor, constraint_violations: torch.Tensor) -> torch.Tensor:
    ...
    def batched_cost_eval(self, trajectory: torch.Tensor, actions: torch.Tensor, constraint_violations: torch.Tensor) -> torch.Tensor:
        """Compute the cost function for multiple trajectory candidates in parallel.
    ...
    def action_info(self, state: torch.Tensor) -> tuple[torch.Tensor, any]:
        ...
        for i in tqdm(range(self.num_gradient_steps)):
            ...
            costs_train = self.batched_cost_train(trajectory, actions, constraint_violations)
            training_losses.append(costs_train.detach().min().item())
            costs_eval = self.batched_cost_eval(trajectory, actions, constraint_violations)
            eval_losses.append(costs_eval.detach().min().item())
            print(training_losses[-1])
            # Backward pass through costs of all trajectories
            if i < self.num_gradient_steps - 1:
                costs_train.sum().backward()
                optimizer.step()
                optimizer.zero_grad()
...
class ErgodicMPCPolicy(ShootingMPCPolicy):
    ...
    def __init__(self, 
        ...
        qs_eval: torch.Tensor,
        bs_eval: torch.Tensor,
        qs_train: torch.Tensor,
        bs_train: torch.Tensor,
        soft_constraint_count_penalty: float=1000., 
        soft_constraint_quad_penalty: float=1000.,
        num_restarts: int=100,
        num_gradient_steps: int=1000,
        ...
        self.qs_eval = qs_eval  # (num_halfspaces, 2)
        self.bs_eval = bs_eval  # (num_halfspaces, 1)
        self.qs_train = qs_train  # (num_halfspaces, 2)
        self.bs_train = bs_train  # (num_halfspaces, 1)
        self.soft_constraint_count_penalty = soft_constraint_count_penalty
        self.soft_constraint_quad_penalty = soft_constraint_quad_penalty
        
        # Compute target probability contents from samples
        self.target_prob_contents_train = self._compute_target_prob_contents_train()
        self.target_prob_contents_eval = self._compute_target_prob_contents_eval()
    
    def _compute_target_prob_contents_train(self) -> torch.Tensor:
        ...
        linear_combinations = torch.matmul(self.qs_train, self.target_density_samples) + self.bs_train  # (num_halfspaces, num_samples)
    ...
    def compute_1d_distance_train(self, trajectory: torch.Tensor) -> torch.Tensor:
        ...
        linear_combinations = torch.einsum('hd,rtd->hrt', self.qs_train, xy_by_restart) + self.bs_train.unsqueeze(1)  # (H,R,T1)
        ...
        target_probs = self.target_prob_contents_train.unsqueeze(1)  # (H,1)
    ...
    def batched_cost_train(self, trajectory: torch.Tensor, actions: torch.Tensor, constraint_violations: torch.Tensor) -> torch.Tensor:
        ...
        ergodic_cost = self.compute_1d_distance_train(trajectory)
    ...
    ###########################################################
    def _compute_target_prob_contents_eval(self) -> torch.Tensor:
        """Compute target probability contents from target density samples.
        ...
        # Compute linear combinations for target samples
        # target_density_samples is (2, num_samples), qs is (num_halfspaces, 2)
        linear_combinations = torch.matmul(self.qs_eval, self.target_density_samples) + self.bs_eval  # (num_halfspaces, num_samples)
    ...
    def compute_1d_distance_eval(self, trajectory: torch.Tensor) -> torch.Tensor:
        """Compute 1D distance between trajectory and target distribution.
        ...
        xy_by_restart = xy_states.permute(1, 0, 2)
        # qs: (H, 2), bs: (H,1)
        H = self.qs_eval.shape[0]
        R = xy_by_restart.shape[0]
        T1 = xy_by_restart.shape[1]
        # Compute projections for all half-spaces, restarts, and time points: (H, R, T1)
        # Using einsum: 'hd,rtd->hrt'
        linear_combinations = torch.einsum('hd,rtd->hrt', self.qs_eval, xy_by_restart) + self.bs_eval.unsqueeze(1)  # (H,R,T1)
        # Differentiable half-space membership
        halfspace_membership = torch.sigmoid(100 * linear_combinations)  # (H,R,T1)
        # Empirical probs per restart per half-space: average over time samples
        empirical_probs = halfspace_membership.mean(dim=2)  # (H,R)
        # Target probs shape (H,) -> (H,1)
        target_probs = self.target_prob_contents_eval.unsqueeze(1)  # (H,1)
        prob_diff = torch.abs(empirical_probs - target_probs)  # (H,R)
        distances = prob_diff.mean(dim=0)  # (R,)
        return distances
    ...
    def batched_cost_eval(self, trajectory: torch.Tensor, actions: torch.Tensor, constraint_violations: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Compute ergodic distance to target distribution
        ergodic_cost = self.compute_1d_distance_eval(trajectory)
